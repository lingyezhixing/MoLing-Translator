...
def translator_management(config_use):
    ...
    if 'sha156_list' not in st.session_state:
        st.session_state['sha256_list'] = []
    if 'file_info_dict' not in st.session_state:
        st.session_state['file_info_dict'] = {}
    
    st.session_state['sha256_list'] = os.listdir(".\Cache\\Temp")
    st.session_state['file_info_dict'] = {}
    
    # 获取文件信息
    for sh256 in st.session_state['sha256_list']:
        with open(".\Cache\\Temp\\" + sh256 + "\\file_info.txt", "r", encoding="utf-8") as f:
            ...
            type = file_info[3]
        st.session_state['file_info_dict'][sh256] = {
            "file_name": file_name,
            "file_path": file_path,
            "total_len": total_len,
            "type": type
    ...
    with open("Cache\\translating_file_sh256.pkl", "rb") as f:
        ...
        if translating_file_sh256 == "model_only":
            translating_file_sh256 = None
    ...
    if start_all_button:
        if translating_file_sh256 == None:
            if len(st.session_state['sha256_list']) > 0:
                translating_file_sh256 = st.session_state['sha256_list'][0]
                with open("Cache\\Translating_info.pkl", "wb") as f:
                    pickle.dump({
                        "file_path": st.session_state['file_info_dict'][translating_file_sh256]['file_path'],
                        "file_type": st.session_state['file_info_dict'][translating_file_sh256]['type'],
                        "config": config_use}, f)
                translation_state = True
                with open("Cache\\translating_file_sh256.pkl", "wb") as f:
                    pickle.dump(translating_file_sh256, f)
    ...
    if 'progress_bars' not in st.session_state:
        st.session_state.progress_bars = {bar: False for bar in st.session_state['sha256_list']}
    if 'markdown_texts' not in st.session_state:
        st.session_state.markdown_texts = {markdown: False for markdown in st.session_state['sha256_list']}
    if 'start_buttons' not in st.session_state:
        st.session_state.start_buttons = {button: False for button in st.session_state['sha256_list']}
    if 'delete_buttons' not in st.session_state:
        st.session_state.delete_buttons = {button: False for button in st.session_state['sha256_list']}
    
    with translator_management_page.container():
        # 展示正在翻译的文件
        if translation_state:
            with st.expander(f"**正在翻译**： {st.session_state['file_info_dict'][translating_file_sh256]['file_name']}", expanded=True):
                col1, col2 = st.columns([4, 1])
                with col1:
                    st.markdown(f"**{st.session_state['file_info_dict'][translating_file_sh256]['file_name']}**")

                    # 获取翻译进度
                    if st.session_state['file_info_dict'][translating_file_sh256]['type'] == "TXT":
                    ...
                    elif st.session_state['file_info_dict'][translating_file_sh256]['type'] == "EPUB":
                    ...
                    # 计算翻译进度
                    progress = translated_texts_count / st.session_state['file_info_dict'][translating_file_sh256]['total_len'] * 100
        ...
        for sh256 in st.session_state['sha256_list']:
            ...
            if translation_state:
                ...
                if n == 1:
                    ...
                    expanded = False
            with st.expander(f"**{state}**： {st.session_state['file_info_dict'][sh256]['file_name']}", expanded=expanded):
                col1, col2 = st.columns([4, 1])
                with col1:
                    st.markdown(f"**{st.session_state['file_info_dict'][sh256]['file_name']}**")

                    # 获取翻译进度
                    if st.session_state['file_info_dict'][sh256]['type'] == "TXT":
                        if os.path.exists(os.path.join(".\Cache\TXT", sh256, "translated_texts")):
                            ...
                            translated_texts_count = 0
                    elif st.session_state['file_info_dict'][sh256]['type'] == "EPUB":
                    ...
                    # 计算翻译进度
                    progress = translated_texts_count / st.session_state['file_info_dict'][sh256]['total_len'] * 100
                ...
                with col2:
                    ...
                    if st.session_state.delete_buttons[sh256]:
                        file_name = st.session_state['file_info_dict'][sh256]['file_name']
                        # 从缓存目录中删除任务
                        if st.session_state['file_info_dict'][sh256]['type'] == "TXT":
                            # 删除缓存文件夹
                        ...
                        # 从st.session_state['sha256_list']和st.session_state['file_info_dict']中删除任务
                        st.session_state['sha256_list'].remove(sh256)
                        del st.session_state['file_info_dict'][sh256]
                        # 刷新页面
                        translator_management_page.empty()
                        st.session_state['navigation'] = '文件翻译'
                        st.rerun()
        ...
        if translation_state:
            ...
            while translating_file_sh256 == last_translating_file_sh256 and translation_state:
                ...
                if translated_texts_count - num > 0:
                    remaining_time = (st.session_state['file_info_dict'][translating_file_sh256]['total_len'] - translated_texts_count) * (time.time() - start_time) / (translated_texts_count - num)
                ...
                if translated_texts_count >= st.session_state['file_info_dict'][translating_file_sh256]['total_len']:
                    st.session_state.markdown_texts[translating_file_sh256].markdown("**正在整合文本并删除缓存文件**")
                    file_name = st.session_state['file_info_dict'][translating_file_sh256]['file_name']
                    file_type = st.session_state['file_info_dict'][translating_file_sh256]['type']
                    ...
                    # 从st.session_state['sha256_list']和st.session_state['file_info_dict']中删除任务
                    st.session_state['sha256_list'].remove(translating_file_sh256)
                    del st.session_state['file_info_dict'][translating_file_sh256]
