    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=TIMEOUT) as session:
            ...
            for i, paragraph in epub_texts:
                ...
                logging.debug(f"EPUB翻译提示词：{preset_prompt_temp}")

                task = asyncio.ensure_future(bound_fetch(sha256, semaphore, session, server, i, system_prompt, preset_prompt_temp, paragraph, model, temperature, top_p, frequency_penalty, max_retry_count, total_completion_tokens = 0))
                tasks.append(task)
